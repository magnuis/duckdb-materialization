...
def prepare_database(con: duckdb.DuckDBPyConnection, fields: list[tuple[str, dict, bool]]):
    ...
    for field, query, materialize in fields:
        changed |= _alter_table(con=con, field=field,
...
def _alter_table(con: duckdb.DuckDBPyConnection, field: str, query: dict, materialize: bool):
    ...
    result = con.execute(
        f"SELECT column_name FROM information_schema.columns WHERE table_name = 'test_table' AND column_name = '{field}'").fetchall()
    ...
    if materialize and not field_exists:
        ...
        con.execute(
            f"ALTER TABLE test_table ADD COLUMN {field} {query['type']};")
        # Update the column with the value from raw_json
        con.execute(f"UPDATE test_table SET {field} = {query['query']};")
        return True
